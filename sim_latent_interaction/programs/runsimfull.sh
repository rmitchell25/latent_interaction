			...
			for LOADLOOP in {0..1}; do #2
				for ITEMLOOP in {0..1}; do #2
					...
					then
						...
						export RUNONCLUSTER PROBLOOP RSQLOOP NLOOP LOADLOOP ITEMLOOP
						/bin/bash ${PROGDIR}/runsim.sh
					else
						
						qsub -v RUNONCLUSTER=${RUNONCLUSTER},PROBLOOP=${PROBLOOP},RSQLOOP=${RSQLOOP},NLOOP=${NLOOP},LOADLOOP=${LOADLOOP},ITEMLOOP=${ITEMLOOP} < runsim.sh 
					fi
				done
    		done
		done
