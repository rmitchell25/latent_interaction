				...
				for LOADLOOP in {0..1}; do #2
					for ITEMLOOP in {0..1}; do #2
						...
						then
							...
							#qsub -v RUNONCLUSTER=${RUNONCLUSTER},CATLOOP=${CATLOOP},PROBLOOP=${PROBLOOP},RSQLOOP=${RSQLOOP},NLOOP=7,LOADLOOP=${LOADLOOP},ITEMLOOP=${ITEMLOOP} < runsim.sh 

							#echo "RUNONCLUSTER=${RUNONCLUSTER},CATLOOP=${CATLOOP},PROBLOOP=${PROBLOOP},RSQLOOP=${RSQLOOP},NLOOP=7,LOADLOOP=${LOADLOOP},ITEMLOOP=${ITEMLOOP}"
						fi
					done
	    		done
			done
