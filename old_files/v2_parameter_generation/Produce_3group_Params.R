...
for (p in 1:3) {
  ...
  for (rsq_product in rsq_prod) {
    ...
    id <- c(paste0("b",0:5),"res_var")
    params <- cbind.data.frame(id,as.numeric(moderated_params), p, rsq_product)
    
    three_group_parameters <- rbind(three_group_parameters, params)
  }
}

colnames(three_group_parameters) <- c("param_id","param","prob","rsq_p")
three_group_parameters <- as.data.frame(three_group_parameters)


save(three_group_parameters, file = "three_group_structural_parameters.rda")
