---
title: "Power & TIE plots"
output: pdf_document
date: "2026-03-12"
---

```{r}
library(ggplot2)
library(dplyr)
options(scipen = 999)


# Load data ----
setwd("/Users/remus/Documents/GitHub/latent_interaction/post_sim_review/three_group_20_rep_test")
load(file = "agg_results.rda")


# Get unique combinations of rsq_prod and n_items
combinations <- agg_results %>%
  distinct(categories, rsq_prod, n_items)

agg_results <- agg_results %>% filter(param.id == "beta_XG2" | param.id == "beta_XG3")


# Loop over each combination
for (i in 1:nrow(combinations)) {
  
  plot_data <- agg_results %>%
    filter(rsq_prod == combinations$rsq_prod[i], n_items  == combinations$n_items[i],
           categories == combinations$categories[i])
  
  # Skip if no data
  if (nrow(plot_data) == 0) next
  
  plot_data <- plot_data %>%
    mutate(
      group_prob = factor(group_prob, levels = c(1, 2, 3)),
      loading    = factor(loading),
      model_type = factor(model_type, levels = c(1, 2, 3),
                    labels = c("FR", "MG-SEM", "Sum Score")),
      sample_size = factor(sample_size) ) 
  
  
p <- ggplot(plot_data, aes(x = sample_size, y = sig,
                           color = model_type,
                           group = model_type)) +
  geom_line() + geom_point() +
  facet_grid(rows = vars(loading),
             cols = vars(group_prob),
             drop = FALSE,
             labeller = labeller(
               loading = function(x) paste("Loading =", x),
               group_prob = function(x) {
                 x_new <- ifelse(x == 1, "Balanced",
                          ifelse(x == 2, "Moderately Unbalanced",
                          ifelse(x == 3, "Severely Unbalanced", x)))
                 paste("Group balance =", x_new)
               }
             )) +
  scale_color_brewer(palette = "Set1", name = "Model Type") +
  labs(
    title = paste0("Rejection Rate | RSQ_prod = ", combinations$rsq_prod[i],
                   ", items = ", combinations$n_items[i]),
    x = "Sample Size", y = "Rejection Rate") +
  theme_bw() +
  theme(
    plot.title       = element_text(hjust = 0.5, face = "bold"),
    legend.position  = "bottom",
    axis.text.x      = element_text(angle = 45, hjust = 1),
    strip.background = element_rect(fill = "lightblue"),
    strip.text       = element_text(face = "bold"))

  
  print(p)
}
```
