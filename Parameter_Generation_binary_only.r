...
run_simulation <- function(parameter_values, group_probs, N, reps = 1000) {
  ...
  for (cond_name in names(parameter_values)) {
    
    cond <- parameter_values[[cond_name]]
    
    moments_G1 <- cond$G1
    moments_G2 <- cond$G2
    moderated_params <- cond$mod_parameters
    
    # --- Recover condition info ---
    parts <- strsplit(cond_name, "_")[[1]]
    prob_idx <- as.numeric(gsub("prob", "", parts[1]))
    probs <- group_probs[prob_idx, ]
    items <- as.numeric(gsub("items", "", parts[3]))
    ...
    for (r in 1:reps) {
      ...
      g1dat <- cbind(
        1,
        rmvnorm(ng[1],
                as.vector(moments_G1$mean),
                as.matrix(moments_G1$covariance))
      )
      
      g2dat <- cbind(
        2,
        rmvnorm(ng[2],
                as.vector(moments_G2$mean),
                as.matrix(moments_G2$covariance))
      )
      ...
      syntax <- list(
        structural_model = c(
          'Y ~ 1@beta0 G@beta_g X@beta_X G*X@beta_XG',
          'Y ~~ Y@resvar'
        ),
        X_measurement = c(
          'X ~~ X@1',
          paste0('X -> ', paste0('X', 1:items, collapse = ' '))
        ),
        Y_measurement = c(
          paste0('Y -> ', paste0('Y', 1:items, collapse = ' ')),
          'Y1 ~ 1@0'
        ),
        predictors = c(
          'X ~ 1@0',
      ...
      tryCatch({
        ...
        mymodel <- rblimp(
          ...
          parameters = c(
            'lx1 ~ trunc(0,1)',
            paste0('beta0_bi = beta0 - ',     round(moderated_params$beta_0, 6)),
            paste0('beta_g_bi = beta_g - ',   round(moderated_params$beta_G, 6)),
            paste0('beta_X_bi = beta_X - ',   round(moderated_params$beta_X, 6)),
            paste0('beta_XG_bi = beta_XG - ', round(moderated_params$beta_XG, 6)),
            paste0('resvar_bi = resvar - ',   round(moderated_params$residual_variance, 6))
          ),
          seed = 90291 + r,
          burn = 20000,
          iter = 20000
        ...
        exclude_names <- grepl("R2|odds ratio|standardized|residual SD|lx1",
                               rownames(mymodel@estimates))
        zero_sd       <- mymodel@estimates[, "StdDev"] == 0
        keep          <- !(exclude_names | zero_sd)
        filtered_est  <- mymodel@estimates[keep, ]
        
        psr_max       <- max(mymodel@psr[20, ], na.rm = TRUE)
        converge_fail <- ifelse(psr_max > 1.05, 1, 0)
        neff_min      <- min(filtered_est[, 7], na.rm = TRUE)
        neff_fail     <- ifelse(neff_min < 100, 1, 0)
        
        psr_neff <- rbind(psr_neff,
                          c(psr_max, converge_fail, neff_min, neff_fail))
        
        focal_rows <- c(
          grep("^Y ~ Intercept$", rownames(mymodel@estimates)),
          grep("^Y ~ G\\.2$", rownames(mymodel@estimates)),
          grep("^Y ~ X$", rownames(mymodel@estimates)),
          grep("^Y ~ G\\.2\\*X$", rownames(mymodel@estimates)),
          grep("^Y residual variance$", rownames(mymodel@estimates))
        )
        
        sig_tmp <- as.numeric(
          mymodel@estimates[focal_rows, "PValue"] < .05
        )
        
        bi_rows <- grep("_bi$", rownames(mymodel@estimates))
        
        results_tmp <- cbind(
          r,
          cond_name,
          1:5,
          mymodel@estimates[bi_rows, 1:2],
          sig_tmp
        )
  ...
  colnames(psr_neff) <- c('psr_max', 'converge_fail', 'neff_min', 'neff_fail')
  
  colnames(results) <- c('rep', 'condition', 'param','bias', 'sd', 'sig')
